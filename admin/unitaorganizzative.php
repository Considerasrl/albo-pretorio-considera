...
$albopc_messages[7] = __('Impossibile cancellare Unità organizzative che sono collegati ad Atti','albo-pretorio-considera');
$albopc_messages[9] = __('Bisogna assegnare il nome alla nuova Unità organizzative','albo-pretorio-considera');
...
if ($albopc_lista){
	foreach($albopc_lista as $albopc_riga){
		echo'<li style="text-align:left;padding-left:1px;">';
	 	$albopc_Tab=0;
		$albopc_Testo_da=__("Confermi la cancellazione dell'Unità Organizzativa","albo-pretorio-considera")." ".stripslashes($albopc_riga->Nome). "?\n\n".__("Sei sicuro di voler proseguire con la CANCELLAZIONE?","albo-pretorio-considera");
		...
		echo '					<a href="?page=unitao&amp;action=edit-unitao&amp;id='.esc_attr($albopc_riga->IdUO).'&amp;modificaunitao='.esc_attr(wp_create_nonce('ediunitao')).'" rel="'.esc_attr(stripslashes($albopc_riga->Nome)).'">
					<span class="dashicons dashicons-edit" title="'.esc_html__("Modifica Unità Organizzativa","albo-pretorio-considera").'" style="margin-left:'.(int)$albopc_Tab.'px;"></span>
					</a>';
...
if($albopc_edit) {
	if(isset($albopc_risultato->Nome)){
		echo '<input type="submit" name="SaveData" id="SaveData" class="button" value="'. esc_attr(__("Memorizza Modifiche Unità Organizzativa","albo-pretorio-considera").' '.(isset($albopc_risultato->Nome)?albopc_sanifica_testo($albopc_risultato->Nome):"")).'" rel="'.esc_attr((isset($albopc_risultato->Nome)?albopc_sanifica_testo($albopc_risultato->Nome):"")).'" />';
	}
}else{
