...
if ($lista){
	foreach($lista as $riga){
		...
		echo '<strong>'.esc_html(stripslashes($riga->Nome)).'</strong> (n&ordm; atti '.(int)(ap_num_enti_atto($riga->IdUO)).')';
		echo '</li>';
	}
} else {
...
if($edit) {
	if(isset($risultato->Nome)){
		echo '<input type="submit" name="SaveData" id="SaveData" class="button" value="'. esc_attr(__("Memorizza Modifiche Unità Organizzativa","albo-pretorio-considera").' '.(isset($risultato->Nome)?ap_sanifica_testo($risultato->Nome):"")).'" rel="'.esc_attr((isset($risultato->Nome)?ap_sanifica_testo($risultato->Nome):"")).'" />';
	}
}else{
