...
$albopc_messages[7] = __('Impossibile cancellare Soggetti che sono collegati ad Atti','albo-pretorio-considera');
	...
	if (!isset($_REQUEST['cancresp'])) {
		...
		if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['cancresp'])),'deleteresponsabile')){
			...
			if(isset($albopc_SoggettiAtti[(isset($_REQUEST['id'])?intval($_REQUEST['id']):0)]) And $albopc_SoggettiAtti[(isset($_REQUEST['id'])?intval($_REQUEST['id']):0)]>0){
				$albopc_NC=__('Impossibile cancellare Soggetti che sono collegati ad Atti','albo-pretorio-considera');
			}else{
...
if ($albopc_lista){
	foreach($albopc_lista as $albopc_riga){
		...
		echo '
			...
			<span class="dashicons dashicons-edit" title="'.esc_attr__("Modifica soggetto","albo-pretorio-considera").'" style="margin-left:'.(int)$albopc_Tab.'px;"></span>
			</a>
