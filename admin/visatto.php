...
function Visualizza_Atto($Parametri){
	...
	if (isset($Parametri['numero']) And is_numeric($Parametri['numero'])){
		...
		if(isset($_GET["numero"]) And is_numeric($_GET["numero"])){
			...
			echo __("Parametro Numero Atto non impostato","albo-pretorio-considera");
			return ob_get_clean();		
		}
	}
	if (isset($Parametri['anno']) And is_numeric($Parametri['anno'])){
		...
		if(isset($_GET["anno"]) And is_numeric($_GET["anno"])){
			...
			echo __("Parametro Anno Atto non impostato","albo-pretorio-considera");
			return ob_get_clean();
		}
	}
	$risultato=ap_get_all_atti(0,$Numero,$Anno);
	if(count($risultato)==0){
		echo __("Nessun atto trovato con questi parametri","albo-pretorio-considera");
		return ob_get_clean();
	}
...
if(count($documenti)>0){
	...
	foreach ($documenti as $allegato) {
		...
		echo '<div class="Visallegato">
				...
				<div>
					<p class="secondaColonna">'.($allegato->DocIntegrale!="1"?'<span class="evidenziato">'.__("Pubblicato per Estratto","albo-pretorio-considera")."</span><br />":"").'<strong>'.__("Descrizione","albo-pretorio-considera").'</strong>: '.wp_strip_all_tags($allegato->TitoloAllegato).'<br /><strong>'.__("Impronta","albo-pretorio-considera").'</strong>: '.$allegato->Impronta.'<br />';
