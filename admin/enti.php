...
$albopc_messages[9] = __('Bisogna assegnare il nome al nuovo ente','albo-pretorio-considera');
...
if ($albopc_lista){
	foreach($albopc_lista as $albopc_riga){
		echo'<li style="text-align:left;padding-left:1px;">';
	 	$albopc_Tab=0;
		$albopc_Testo_da=__("Confermi la cancellazione dell'Ente","albo-pretorio-considera")." ".albopc_sanifica_testo($albopc_riga->Nome). "?\n\n".__("Sei sicuro di voler proseguire con la CANCELLAZIONE?","albo-pretorio-considera");
		...
		echo '					<a href="?page=enti&amp;action=edit-ente&amp;id='.(int)$albopc_riga->IdEnte.'&amp;modificaente='.esc_attr(wp_create_nonce('editente')).'" rel="'.esc_attr(albopc_sanifica_testo($albopc_riga->Nome)).'">
					<span class="dashicons dashicons-edit" title="'.esc_attr__("Modifica ente","albo-pretorio-considera").'" style="margin-left:'.(int)$albopc_Tab.'px;"></span>
					</a>';
...
<div class="form-wrap">
	...
	<form id="addtag" method="post" action="?page=enti" class="<?php if($albopc_edit) echo "edit"; else echo "validate"; ?>"  >
		...
		<div class="form-field"  style="margin-bottom:0px;margin-top:0px;">
			...
			<input name="ente-indirizzo" id="ente-indirizzo" type="text" value="<?php if($albopc_edit) echo isset($albopc_risultato->Indirizzo)?esc_attr(albopc_sanifica_testo($albopc_risultato->Indirizzo)):esc_attr__("Non Definito","albo-pretorio-considera"); else echo isset($_REQUEST['ente-indirizzo'])?esc_attr(albopc_sanifica_testo(sanitize_text_field(wp_unslash($_REQUEST['ente-indirizzo'])))):"";?>" size="150"/>
		</div>
		<div class="form-field form-required"  style="margin-bottom:0px;margin-top:0px;">
			<label for="ente-url"><?php esc_html_e("Url","albo-pretorio-considera");?></label>
			<input name="ente-url" id="ente-url" type="url" value="<?php if($albopc_edit) echo isset($albopc_risultato->Url)?esc_attr(albopc_sanifica_testo($albopc_risultato->Url)):esc_attr__("Non Definito","albo-pretorio-considera"); else echo isset($_REQUEST['ente-url'])?esc_attr(albopc_sanifica_testo(sanitize_text_field(wp_unslash($_REQUEST['ente-url'])))):"";?>" size="100"/>
		</div>
		<div class="form-field form-required"  style="margin-bottom:0px;margin-top:0px;">
			<label for="ente-email"><?php esc_html_e("Email","albo-pretorio-considera");?> <span style="color:red;font-weight: bold;">*</span></label>
			<input name="ente-email" id="<?php esc_html_e("Email","albo-pretorio-considera");?>" type="email" value="<?php if($albopc_edit) echo isset($albopc_risultato->Email)?esc_attr(albopc_sanifica_testo($albopc_risultato->Email)):esc_attr__("Non Definito","albo-pretorio-considera"); else echo isset($_REQUEST['ente-email'])?esc_attr(albopc_sanifica_testo(sanitize_text_field(wp_unslash($_REQUEST['ente-email'])))):"";?>" size="100" alt="Email" required/>
		</div>
		<div class="form-field form-required"  style="margin-bottom:0px;margin-top:0px;">
			<label for="ente-pec"><?php esc_html_e("Pec","albo-pretorio-considera");?> <span style="color:red;font-weight: bold;">*</span></label>
			<input name="ente-pec" id="<?php esc_html_e("Pec","albo-pretorio-considera");?>" type="email" value="<?php if($albopc_edit) echo isset($albopc_risultato->Pec)?esc_attr(albopc_sanifica_testo($albopc_risultato->Pec)):esc_attr__("Non Definito","albo-pretorio-considera"); else echo isset($_REQUEST['ente-pec'])?esc_attr(albopc_sanifica_testo(sanitize_text_field(wp_unslash($_REQUEST['ente-pec'])))):"";?>" size="100" alt="Pec" required/>
		</div>
		<div class="form-field"  style="margin-bottom:0px;margin-top:0px;">
			<label for="ente-telefono"><?php esc_html_e("Telefono","albo-pretorio-considera");?></label>
			<input name="ente-telefono" id="ente-telefono" type="text" value="<?php if($albopc_edit) echo isset($albopc_risultato->Telefono)?esc_attr(albopc_sanifica_testo($albopc_risultato->Telefono)):esc_attr__("Non Definito","albo-pretorio-considera"); else echo isset($_REQUEST['ente-telefono'])?esc_attr(albopc_sanifica_testo(sanitize_text_field(wp_unslash($_REQUEST['ente-telefono'])))):"";?>" size="30"/>
		</div>
		<div class="form-field"  style="margin-bottom:0px;margin-top:0px;">
			<label for="ente-fax"><?php esc_html_e("Fax","albo-pretorio-considera");?></label>
			<input name="ente-fax" id="ente-fax" type="text" value="<?php if($albopc_edit) echo isset($albopc_risultato->Fax)?esc_attr(albopc_sanifica_testo($albopc_risultato->Fax)):esc_attr__("Non Definito","albo-pretorio-considera"); else echo isset($_REQUEST['ente-fax'])?esc_attr(albopc_sanifica_testo(sanitize_text_field(wp_unslash($_REQUEST['ente-fax'])))):"";?>" size="30"/>
		</div>
		<div class="form-field"  style="margin-bottom:0px;margin-top:0px;">
			<label for="tag-description"><?php esc_html_e("Note","albo-pretorio-considera");?></label>
			<textarea name="ente-note" id="ente-note" rows="5" cols="40"><?php if($albopc_edit) echo isset($albopc_risultato->Note)?esc_textarea(albopc_sanifica_areatesto($albopc_risultato->Note)):esc_html__("Non Definito","albo-pretorio-considera"); else echo isset($_REQUEST['ente-note'])?esc_textarea(albopc_sanifica_areatesto(sanitize_textarea_field(wp_unslash($_REQUEST['ente-note'])))):"";?></textarea>
			<p><?php esc_html_e("inserire eventuali informazioni aggiuntive","albo-pretorio-considera");?></p>
		</div>

<?php
if($albopc_edit) {
	echo '<input type="submit" name="SaveData" id="SaveData" class="button" value="'. esc_attr(__("Memorizza Modifiche Ente","albo-pretorio-considera").' '.stripslashes($albopc_risultato->Nome)).'" rel="'.esc_attr(stripslashes($albopc_risultato->Nome)).'" />';
}else{
 	if (isset($_REQUEST['action']) And $_REQUEST['action']=="edit_err")
		echo '<input type="submit" name="SaveData" id="SaveData" class="button" value="'. esc_attr(__("Memorizza Modifiche Ente","albo-pretorio-considera").' '.stripslashes($albopc_risultato->Nome)).'" rel="'.esc_attr(sanitize_text_field(wp_unslash($_REQUEST['ente-nome']))).'" />';
	else
