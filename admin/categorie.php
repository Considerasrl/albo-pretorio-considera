...
$albopc_messages[7] = __('Impossibile cancellare Categorie che contengono Categorie Figlio. Cancellare prima i Figli','albo-pretorio-considera');
$albopc_messages[8] = __('Impossibile cancellare Categorie che sono collegate ad Atti','albo-pretorio-considera');
$albopc_messages[9] = __('Bisogna assegnare il nome alla nuova categoria','albo-pretorio-considera');
...
if ($albopc_lista){
	foreach($albopc_lista as $albopc_riga){
	 ...
	 $albopc_Tab=0;
	 $albopc_Testo_da=__("Confermi la cancellazione della Categoria","albo-pretorio-considera")." ".albopc_sanifica_testo($albopc_riga[1]). "?\n\n".__("Sei sicuro di voler proseguire con la CANCELLAZIONE?","albo-pretorio-considera");
...
';
	 ...
	 echo'					
			...
			<span class="dashicons dashicons-edit" title="'.esc_html__("Modifica categoria","albo-pretorio-considera").'" style="margin-left:'.(int)$albopc_Tab.'px;"></span>
			</a>
			('.esc_html($albopc_riga[0]) .') '.esc_html($albopc_riga[1]) .' (n&ordm; atti '.(int)(albopc_num_atti_categoria($albopc_riga[0])).')
			</li>'; 
	}
} else {
		echo '<li>'.esc_html__("Nessuna Categoria Codificata","albo-pretorio-considera").'</li>';
}
echo '</ul>
		</td>
	 </tr>
...
<div class="form-wrap">
	<form id="addtag" method="post" action="?page=categorie" class="<?php if($albopc_edit) echo "edit"; else echo "validate"; ?>"  >
		...
		<div class="form-field form-required">
			...
			<input name="cat-name" id="<?php esc_html_e("Nome","albo-pretorio-considera");?>" type="text" value="<?php if($albopc_edit) echo esc_attr(albopc_sanifica_testo($albopc_risultato[0]->Nome)); ?>" size="40" request/>
		...
		<div class="form-field  form-required">
			...
			<input name="cat-durata" id="<?php esc_html_e("Durata","albo-pretorio-considera");?>" type="number" minval=0 value="<?php if($albopc_edit) echo intval($albopc_risultato[0]->Giorni); else echo "0"; ?>" size="4" style="width:6em;" alt="<?php esc_html_e("Durata Atto","albo-pretorio-considera");?>" required />
...
<?php
if($albopc_edit) {
	echo '<input type="submit" name="SaveData" id="SaveData" class="button" value="'. esc_attr(__("Memorizza Modifiche Categoria","albo-pretorio-considera").' '.albopc_sanifica_testo($albopc_risultato[0]->Nome)).'" rel="'.esc_attr(albopc_sanifica_testo($albopc_risultato[0]->Nome)).'" />';
}else{
